...
class PacketTunnelProvider: NEPacketTunnelProvider, WireGuardAdapterDelegate {
    func adapterShouldReassert(_ adapter: WireGuardKit.WireGuardAdapter, reasserting: Bool) {
        if reasserting {
            log("Reasserting tunnel")
        } else {
            log("Tunnel reassertion completed")
        }
    }

    func adapterShouldSetNetworkSettings(_ adapter: WireGuardKit.WireGuardAdapter, settings: NEPacketTunnelNetworkSettings, completionHandler: (((any Error)?) -> Void)?) {
        setTunnelNetworkSettings(settings) { error in
            if let error = error {
                self.log("Failed to set tunnel network settings: \(error.localizedDescription)")
            }
    ...
    private lazy var adapter: WireGuardAdapter = {
        return WireGuardAdapter(with: self, backend: VpnBackend()) { [weak self] _, message in
            self?.log(message)
        }
    }()
...
class VpnBackend: WireGuardBackend {
    private let backend: WireGuardBackend

    init() {
        self.backend = WireGuardBackendGo()
    }

    func setLogger(context: UnsafeMutableRawPointer?, logger_fn: WireGuardKit.WireGuardLoggerCallback?) {
        backend.setLogger(context: context, logger_fn: logger_fn)
    }

    func turnOn(settings: String, tun_fd: Int32) -> Int32 {
        return backend.turnOn(settings: settings, tun_fd: tun_fd)
    }
    ...
    func setConfig(_ handle: Int32, settings: String) -> Int64 {
        return backend.setConfig(handle, settings: settings)
    }

    func getConfig(_ handle: Int32) -> String? {
        return backend.getConfig(handle)
    }
